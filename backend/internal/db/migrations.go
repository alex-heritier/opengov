...
CREATE INDEX IF NOT EXISTS idx_policy_documents_unique_key ON policy_documents(unique_key);
...
func (db *DB) addMissingColumns(tx *sql.Tx) error {
	...
	err := tx.QueryRow("SELECT COUNT(*) FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'policy_documents' AND column_name = 'source'").Scan(&count)
	if err != nil {
		return err
	}
	if count == 0 {
		_, err = tx.Exec("ALTER TABLE policy_documents ADD COLUMN source TEXT NOT NULL DEFAULT 'federal_register'")
		if err != nil && !strings.Contains(err.Error(), "already exists") {
			return err
		}

		_, err = tx.Exec("ALTER TABLE policy_documents ADD COLUMN source_id TEXT NOT NULL DEFAULT ''")
		if err != nil && !strings.Contains(err.Error(), "already exists") {
			return err
		}

		_, err = tx.Exec("ALTER TABLE policy_documents ADD COLUMN unique_key TEXT")
		if err != nil && !strings.Contains(err.Error(), "already exists") {
			return err
		}

		_, err = tx.Exec("UPDATE policy_documents SET source = 'federal_register', source_id = document_number, unique_key = 'federal_register:' || document_number WHERE source IS NULL OR source = ''")
		if err != nil {
			return fmt.Errorf("failed to backfill source columns: %w", err)
		}

		_, err = tx.Exec("CREATE UNIQUE INDEX IF NOT EXISTS idx_policy_documents_unique_key ON policy_documents(unique_key)")
		if err != nil && !strings.Contains(err.Error(), "already exists") {
			return fmt.Errorf("failed to create unique_key index: %w", err)
		}
	}

	err = tx.QueryRow("SELECT COUNT(*) FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'policy_documents' AND column_name = 'document_type'").Scan(&count)
	if err != nil {
		return err
	}
	if count == 0 {
		_, err = tx.Exec("ALTER TABLE policy_documents ADD COLUMN document_type TEXT")
		if err != nil && !strings.Contains(err.Error(), "already exists") {
			return err
		}
	}

	err = tx.QueryRow("SELECT COUNT(*) FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'policy_documents' AND column_name = 'pdf_url'").Scan(&count)
	if err != nil {
		return err
	}
	if count == 0 {
		_, err = tx.Exec("ALTER TABLE policy_documents ADD COLUMN pdf_url TEXT")
		if err != nil && !strings.Contains(err.Error(), "already exists") {
			return err
		}
	}
	...
	err = tx.QueryRow("SELECT COUNT(*) FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'policy_documents' AND column_name = 'agency'").Scan(&count)
	if err != nil {
		return err
	}
	if count == 0 {
		_, err = tx.Exec("ALTER TABLE policy_documents ADD COLUMN agency TEXT")
		if err != nil && !strings.Contains(err.Error(), "already exists") {
			return err
		}
	}
	_, err = tx.Exec("ALTER TABLE policy_documents DROP COLUMN IF EXISTS department")
	if err != nil && !strings.Contains(err.Error(), "does not exist") {
		return err
	}

	var keypointsType string
	err = tx.QueryRow("SELECT data_type FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'policy_documents' AND column_name = 'keypoints'").Scan(&keypointsType)
	if err != nil && err != sql.ErrNoRows {
		return err
	}
	if keypointsType == "" {
		_, err = tx.Exec("ALTER TABLE policy_documents ADD COLUMN keypoints JSONB")
		if err != nil && !strings.Contains(err.Error(), "already exists") {
			return err
		}
	} else if keypointsType != "jsonb" {
		_, err = tx.Exec("ALTER TABLE policy_documents ALTER COLUMN keypoints TYPE JSONB USING keypoints::jsonb")
		if err != nil && !strings.Contains(err.Error(), "already exists") {
			return err
		}
	}

	err = tx.QueryRow("SELECT COUNT(*) FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'policy_documents' AND column_name = 'impact_score'").Scan(&count)
	if err != nil {
		return err
	}
	if count == 0 {
		_, err = tx.Exec("ALTER TABLE policy_documents ADD COLUMN impact_score TEXT")
		if err != nil && !strings.Contains(err.Error(), "already exists") {
			return err
		}
	}

	err = tx.QueryRow("SELECT COUNT(*) FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'policy_documents' AND column_name = 'political_score'").Scan(&count)
	if err != nil {
		return err
	}
	if count == 0 {
		_, err = tx.Exec("ALTER TABLE policy_documents ADD COLUMN political_score INTEGER")
		if err != nil && !strings.Contains(err.Error(), "already exists") {
			return err
		}
	}
	...
	err = tx.QueryRow("SELECT COUNT(*) FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'policy_documents' AND column_name = 'feed_entry_id'").Scan(&count)
	if err != nil {
		return err
	}
	if count == 0 {
		_, err = tx.Exec("ALTER TABLE policy_documents ADD COLUMN feed_entry_id BIGINT")
		if err != nil && !strings.Contains(err.Error(), "already exists") {
			return err
		}

		_, err = tx.Exec("ALTER TABLE policy_documents ADD CONSTRAINT policy_documents_feed_entry_id_fkey FOREIGN KEY (feed_entry_id) REFERENCES feed_entries(id) ON DELETE CASCADE")
		if err != nil && !strings.Contains(err.Error(), "already exists") {
			return fmt.Errorf("failed to add feed_entry_id foreign key: %w", err)
		}

		_, err = tx.Exec("ALTER TABLE policy_documents ADD UNIQUE (feed_entry_id)")
		if err != nil && !strings.Contains(err.Error(), "already exists") {
	...
	// Add FK + unique constraint to policy_document_sources if missing
	var fkCount int
	err = tx.QueryRow(`
		SELECT COUNT(*) FROM information_schema.table_constraints
		WHERE constraint_type = 'FOREIGN KEY'
		AND table_name = 'policy_document_sources'
		AND constraint_name = 'policy_document_sources_policy_document_id_fkey'
	`).Scan(&fkCount)
	if err != nil {
		return err
	}
	if fkCount == 0 {
		_, err = tx.Exec("ALTER TABLE policy_document_sources ADD CONSTRAINT policy_document_sources_policy_document_id_fkey FOREIGN KEY (policy_document_id) REFERENCES policy_documents(id) ON DELETE CASCADE")
		if err != nil && !strings.Contains(err.Error(), "already exists") {
			return fmt.Errorf("failed to add policy_document_sources FK: %w", err)
		}
	}
	...
	if uqCount == 0 {
		_, err = tx.Exec("ALTER TABLE policy_document_sources ADD CONSTRAINT policy_document_sources_source_key_external_id_key UNIQUE (source_key, external_id)")
		if err != nil && !strings.Contains(err.Error(), "already exists") {
			return fmt.Errorf("failed to add policy_document_sources unique constraint: %w", err)
		}
	}

	// Drop redundant non-unique index (unique constraint creates its own index)
	_, err = tx.Exec("DROP INDEX IF EXISTS idx_policy_document_sources_source_key_external_id")
	if err != nil {
		return fmt.Errorf("failed to drop redundant index: %w", err)
	}
