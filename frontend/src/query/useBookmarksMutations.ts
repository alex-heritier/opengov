...
import { useArticleUIStore, type ArticleUIState } from '@/store/article-ui-store'
...
export function useToggleBookmarkMutation() {
  ...
  return useMutation({
    mutationFn: async (articleId: number) => {
      const { data } = await client.post<BookmarkResponse>(`/api/bookmarks/${articleId}`, {})
      return data
    },
...
export function useRemoveBookmarkMutation() {
  ...
  return useMutation({
    mutationFn: async (articleId: number) => {
      const { data } = await client.delete<BookmarkResponse>(`/api/bookmarks/${articleId}`)
      return data
    },
